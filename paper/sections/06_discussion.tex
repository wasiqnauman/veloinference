...
The final measurements exhibit exactly this separation. At 1.0--1.8
requests/s, fixed batching raises call-weighted grouping from 1.54 to 2.90,
but the occupancy prediction explains the grouping with 0.86\% mean absolute
percentage error. Backend time does not fall consistently, while mean gateway
queue delay rises to 0.77--0.83 seconds. The resulting p95 penalties remain
positive under paired run-level intervals. The failure is therefore not that
the gateway cannot aggregate prompts; it aggregates them for the wrong reason
and at the wrong scheduling boundary.
...
\textbf{Hardware, runtime, and order.} The study uses one consumer GPU under
WSL2 and the Windows display-driver model. Absolute latency and host scheduling
may differ on native Linux or datacenter GPUs. Conditions were executed in a
fixed, recoverable order rather than randomized or interleaved, and collection
spanned multiple sessions after documented interruptions. Warmup and cooldown
do not remove slow temporal or thermal confounding. Paired seeds reproduce
workload construction but do not pair wall-clock state. This weakens small
cross-mode comparisons; it is less threatening to the within-run identity
between queue growth, backend occupancy, and outer grouping. GPU monitoring
also lost \texttt{nvidia-smi} access after a restart, so the available samples
cannot support a complete utilization, VRAM, power, or thermal comparison.
...
system optimized for interactive streaming may require those objectives. The
reported requests/s metric is successful request count divided by the fixed
arrival-schedule duration; it verifies completion of offered work but is not a
service-capacity measurement because it ignores drain time after the final
scheduled arrival.
...
research: a negative result with a complete evidentiary trail is more
informative than a large percentage derived from one favorable run. The code,
configs, tests, raw-schema definitions, final aggregate JSON, and analysis
scripts make every number in the paper traceable to named run IDs. Raw traces
remain local until an archival release is prepared; the manuscript does not
claim that a public dataset already exists.
