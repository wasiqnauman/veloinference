...
wait window. The primary workload schedules requests periodically, so its
nominal inter-arrival gap is
\begin{equation}
  \Delta = \lambda^{-1}.
  \label{eq:periodic-gap}
\end{equation}
In the absence of backlog or harness jitter, the number of companions that can
arrive during a window is exactly zero whenever $w<\Delta$. At the largest
tested rate, 1.8 requests/s, $\Delta=555.6$\,ms: both the 1\,ms fixed window
and the 20\,ms adaptive cap close hundreds of milliseconds before the next
scheduled arrival. Thus, the primary workload makes a sharper prediction than
``small batches are unlikely'': intentional waiting alone should form no
multi-request batch.

For comparison, if arrivals were memoryless rather than periodic, a Poisson
approximation gives
\begin{equation}
  ...
  \label{eq:window}
\end{equation}
where $N_w$ is the number of companion arrivals during the window. At
1.8 requests/s, a 1\,ms window yields
$\mathbb{E}[N_w]=0.0018$ and $P(N_w\ge1)\approx0.18\%$. Even a 20\,ms cap
yields only 0.036 expected companions and a 3.54\% probability. The Poisson
case is therefore a conservative stress intuition, not the data-generating
model used for the confirmatory matrix.

Now let $S$ be the time for which a serial gateway worker awaits one backend
call. Requests continue to arrive during that interval, creating on the order
of $\lambda S$ queued companions (up to the phase and integer boundary of the
periodic schedule). When the worker returns and drains the ingress queue, it
can dispatch a multi-request backend call even if $w$ is nearly zero.
...
This gives a quantitative prediction in the continuously busy regime. Let
$\bar S_{\mathrm{call}}$ be mean backend occupancy weighted once per outer
call. The next call's expected group size should track
\begin{equation}
  \widehat B_{\mathrm{occupancy}}
  = \max\!\left(1,\lambda \bar S_{\mathrm{call}}\right),
  \label{eq:occupancy-prediction}
\end{equation}
where the floor at one represents the request that begins a cycle after an idle
period. Agreement with Eq.~\ref{eq:occupancy-prediction}, despite
$w\ll\Delta$, is evidence for backlog coalescing; systematic excess over it
would suggest intentional waiting or residual queue growth.

Our evaluation tests which mechanism dominates by jointly examining observed
batch size, gateway queue time, backend time, completion of offered work, and
tail latency. This mechanism-level accounting is the paper's main distinction
from a benchmark that merely ranks configurations.
