...
batching provide no repeatable throughput or tail-latency advantage over
pass-through at matched offered load. A positive result requires a paired,
run-level improvement that is visible across repetitions; a negative result is
evidence that the engine, not the gateway, should own scheduling for the tested
regime.
...
generation parameters, open-loop arrivals, and one local \gpu{}. Three seeds
serve as the replication unit. We retain request-level latency, queue and
backend decomposition, batch identifiers and sizes, GPU telemetry, failures,
and arrival drift.
...
\begin{enumerate}[leftmargin=1.4em]
  ...
  \item It publishes the complete validity trail, including interrupted and
        harness-rejected attempts, so neutral or negative findings remain
        auditable.
\end{enumerate}
