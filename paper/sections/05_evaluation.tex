...
\begin{figure*}[t]
  ...
  fixed by the open-loop schedule. Outer request grouping is the number of
...
\begin{figure*}[t]
  ...
  \caption{Mechanism accounting for \model{} on one \gpu{}. Panel A separates
  gateway queue and backend time for gateway modes. Hatching denotes queue
  time stacked above backend time. Panel B shows paired p95-latency effects by
  repetition seed; error bars are 95\% Student-$t$ intervals over the three
  paired differences.}
  \label{fig:mechanism}
\end{figure*}
}{}
...
\begin{table}[t]
  ...
  \label{tab:effects}
  \resizebox{\columnwidth}{!}{\input{tables/paired_effects}}
\end{table}
}{}
