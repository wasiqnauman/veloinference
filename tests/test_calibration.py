...
def test_calibration_request_builder_supports_gateway_policy_labels() -> None:
    config = load_experiment_config(ROOT / "configs" / "experiments" / "mock_smoke.toml")
    prompts = build_prompt_set(
        WhitespaceTokenCounter(),
        config.workload.prompt_buckets,
    ...
    requests = build_calibration_requests(config, 1.0, "wait-5-rate-1", prompts, "fixed")

    assert all(request.policy == "fixed" for request in requests)
