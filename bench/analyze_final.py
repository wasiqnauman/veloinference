...
def occupancy_fit_summary(runs: list[RunResult]) -> dict[str, float | int | None]:
    """Summarize descriptive agreement with the serial-occupancy prediction."""

    pairs = [
        (
            float(run.summary["mean_outer_group_size_per_call"]),
            float(run.summary["occupancy_predicted_group_size"]),
        )
        for run in runs
        if run.mode in {"fixed", "adaptive"}
        and run.summary.get("occupancy_predicted_group_size") is not None
    ]
    if not pairs:
        raise ValueError("No batched runs are available for occupancy-fit analysis")
    observed = [pair[0] for pair in pairs]
    ratios = [actual / estimate for actual, estimate in pairs]
    observed_mean = fmean(observed)
    total_variation = sum((value - observed_mean) ** 2 for value in observed)
    residual_variation = sum(
        (actual - estimate) ** 2 for actual, estimate in pairs
    )
    return {
        "n": len(pairs),
        "mean_absolute_error_requests_per_call": fmean(
            abs(actual - estimate) for actual, estimate in pairs
        ),
        "mean_absolute_percentage_error": 100.0
        * fmean(abs(actual - estimate) / estimate for actual, estimate in pairs),
        "r_squared": (
            None
            if math.isclose(total_variation, 0.0)
            else 1.0 - residual_variation / total_variation
        ),
        "fit_ratio_min": min(ratios),
        "fit_ratio_max": max(ratios),
    }
...
def write_analysis_json(
    ...
    payload = {
        ...
        "excluded": excluded,
        "occupancy_fit": occupancy_fit_summary(runs),
...
def plot_overview(
    ...
    """Plot throughput, latency, batching, and gateway queue delay."""
    ...
    for row in aggregates:
        lines.append(
            "{} & {:.1f} & {} & {} & {} & {} \\\\".format(
                ...
                _latex_stat(row["mean_queue_ms"], 0),
            )
        )
